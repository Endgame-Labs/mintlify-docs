...
<Warning>
  **Deprecation:** The Endgame REST API is being deprecated on **October 1,
  2026**. For automated processes and integrations, we recommend the [Endgame
  MCP server](/features/mcp-server) instead.
</Warning>
...
<Note>
  ...
  the existence of threads the caller can't access.
</Note>
...
<Note>
  The default quota is **10,000 requests per organization per day**.
</Note>
...
<Note>
  Requires a user-scoped API key. Org-wide API keys and M2M tokens receive `403
  FORBIDDEN`.
</Note>
...
- When `truncated` is `true`, the walk hit the server-side window before
  ...
  rest. `truncated` is `false` for any org whose thread
...
<Note>
  Requires a user-scoped API key; org-wide API keys and M2M tokens receive `403
  FORBIDDEN`. Users can only rename threads they created -- attempting to rename
...
<Note>
  Requires a user-scoped API key; org-wide API keys and M2M tokens receive `403
  FORBIDDEN`. Users can only delete threads they created.
</Note>
