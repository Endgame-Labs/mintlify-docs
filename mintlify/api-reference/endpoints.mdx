...
<Note>
  Callers can only rename threads they created. Attempting to rename
  another principal's thread returns `403 FORBIDDEN`, even when the
  thread is published.
</Note>
...
<Note>
  Callers can only delete threads they created. Attempting to delete
  another principal's thread returns `403 FORBIDDEN`.
</Note>
