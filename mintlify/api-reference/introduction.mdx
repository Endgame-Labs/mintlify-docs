...
Authenticate with a Bearer token and POST a prompt to `/threads`. The
`prompt` is required -- it's persisted as the first user message and
...
edge for one hour, so it's always in sync with the deployed endpoints. Use
it to generate clients, populate Postman collections, or drive your own
integration tests.

<Note>
  The per-endpoint pages in the sidebar of this reference are rendered from
  a snapshot of the spec vendored into the docs repo, not the live
  `openapi.json` endpoint. The two stay in sync through release review.
</Note>
