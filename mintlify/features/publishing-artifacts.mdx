...
As an Artifact, your Thread will now be able shareable as a link to non-Endgame users as well as be visible to other Endgame users in the Artifacts tab in the Account Detail page if it is account specific. If the thread is multi-account it will be present in the [threads list](https://app.endgame.io/threads) and show a Published label. 
