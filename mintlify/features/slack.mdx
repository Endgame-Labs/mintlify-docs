...
In general, organization-wide channels, EX #marketing, are intended for multi-account inquiries, while account-specific channels, EX #customer-acme, are intended for questions related to individual accounts.

Within any non-external channel that you have added the Endgame app and made an account or organization-wide association, simply type "@Endgame \<your prompt\>" and Endgame will spin up a thread to answer your question.

<Info>
  Connected external Slack channels are read-only for fact generation. Bot chat is disabled to protect internal organization data from external exposure.
</Info>
