---
title: "Zoom"
description: "Process Zoom call recordings and gain insights from your meetings"
icon: "video"
---

Use the instructions below to enable Zoom integration in Endgame. Once enabled, Endgame will process your Zoom call recording data and provide our insights via the Endgame UI.

## Adding the App

<Note>
  Connecting to the Zoom API requires that the connecting user is a Zoom
  Administrator
</Note>

<Steps>
 <Step title="Navigate to Configuration">
    Navigate to the [integrations page](https://app.endgame.io/settings/integrations). Only Endgame Admins can configure organization integrations

    If needed, login to your Endgame account via Salesforce

  </Step>
  <Step title="Start Setup">
    From the integrations page, click "Connect" to kick off the Zoom authentication process.

    <Frame caption="Zoom connection">
      <img
        src="../images/zoom-connection-05-06-26.png"
        alt="Zoom connection"
        className="rounded-lg"
      />
    </Frame>

  </Step>
  <Step title="Grant Access">
    In the resulting page, click "Allow" to grant Endgame access to your Zoom Account

    <Frame>
      <img
        src="/images/zoom-setup-authorization.png"
        alt="Zoom authorization page"
        className="rounded-lg"
      />
    </Frame>

  </Step>
</Steps>

## Usage

That's it! Now that you've connected Zoom to Endgame, we'll automatically ingest your data into our systems and present our insights in Endgame.

Endgame accesses meetings (both past and upcoming), their participants, and recordings.

Within Endgame, this allows us to show meetings that you've had on an account, which opportunities they're associated with, and to help build a robust picture of your interactions with customers and contacts.

## Removing the App

<Steps>
  <Step title="Access Zoom Marketplace">
    Log in to your Zoom account and navigate to the [Zoom App
    Marketplace](https://marketplace.zoom.us/)
  </Step>
  <Step title="Find Endgame App">
    Click Manage → Added Apps, or search for the 'Endgame for Zoom' app
  </Step>
  <Step title="Remove App">Click 'Remove'</Step>
</Steps>

<Info>
  - When the app is removed, Endgame will not surface new meetings or transcript
  information. - Endgame does not automatically remove data when the app is
  removed - contact [support@endgame.io](mailto:support@endgame.io) with data
  removal requests.
</Info>

## Troubleshooting

Connecting to the Zoom API requires that the connecting user is a Zoom Administrator

## Contact Support

We'd love to hear from you! You can reach us at [support@endgame.io](mailto:support@endgame.io).

Support Availability:

- Monday - Friday, 10AM - 4PM Pacific Time, excluding public holidays
- Initial Response SLA: 1 business day
